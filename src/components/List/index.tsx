...
const List = (props:listProps, ref: React.LegacyRef<HTMLDivElement> | undefined) => {
  const {
    style = {},
    className,
    dataSource,
  ...
  const classNames = cs( className, `wsk-${darkTheme ? 'dark-' : ''}list`);
  ...
  useEffect(() => {
    if (lazyLoad && defaultShowNum) {
      ...
      let rowHeight = document.querySelector('.wsk-list-item')?.clientHeight as number;
      switch (size) {
        case 'default':
          rowHeight += 26;
          break;
